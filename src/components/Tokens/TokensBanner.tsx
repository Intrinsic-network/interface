...
const BackgroundColor = styled(Link)<{ show: boolean }>`
  background-color: ${({ theme }) =>
    theme.darkMode ? theme.backgroundScrim : "#FDF0F8"};
...
const PopupContainer = styled.div`
  background-color: ${({ theme }) =>
    theme.darkMode ? theme.backgroundScrim : opacify(60, "#FDF0F8")};
  background-image: url(${({ theme }) =>
    theme.darkMode ? `${tokensPromoDark}` : `${tokensPromoLight}`});
...
export default function TokensBanner() {
  ...
  return showTokensPromoBanner ? (
    <BackgroundColor show={true} to={`/tokens/${chainName}`}>
      <TraceEvent
