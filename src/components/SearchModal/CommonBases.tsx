...
const BaseWrapper = styled.div<{ disable?: boolean }>`
  border: 1px solid
    ${({ theme, disable }) =>
      disable ? theme.intGray : theme.tertiaryButtonColor};
  border-radius: 16px;
  display: flex;
  padding: 6px;
  padding-right: 12px;
  ...
  :hover {
    ...
    background-color: ${({ theme }) => theme.tertiaryAccentColor};
  }

  color: ${({ theme, disable }) => disable && theme.textColor};
  background-color: ${({ theme, disable }) =>
    disable && theme.tertiaryAccentColor};
`;
