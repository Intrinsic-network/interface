...
export function useUnsupportedTokens(): { [address: string]: Token } {
  ...
  const l2InferredBlockedTokens: typeof unsupportedTokens = useMemo(() => {
    ...
    const { current: list } = listsByUrl[listUrl as any]
    if (!list) {
      return {}
    }
    ...
    return list.tokens.reduce((acc: any, tokenInfo: any) => {
      ...
      if (
        bridgeInfo &&
        bridgeInfo[SupportedChainId.RSK_MAINNET] &&
        bridgeInfo[SupportedChainId.RSK_MAINNET].tokenAddress &&
        unsupportedSet.has(bridgeInfo[SupportedChainId.RSK_MAINNET].tokenAddress)
      ) {
        const address = bridgeInfo[SupportedChainId.RSK_MAINNET].tokenAddress
        // don't rely on decimals--it's possible that a token could be bridged w/ different decimals on the L2
        return { ...acc, [address]: new Token(SupportedChainId.RSK_MAINNET, address, tokenInfo.decimals) }
      }
      return acc
    }, {})
