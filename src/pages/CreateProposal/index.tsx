...
export default function CreateProposal() {
  ...
  const [currencyValue, setCurrencyValue] = useState<Currency>(INT[chainId ?? 30])
  ...
  const handleRecipientAddressInput = useCallback(
    (recipientAddress: string) => {
      setRecipientAddressValue(recipientAddress)
    },
  ...
  const isFormInvalid = useMemo(() => {
    ...
    if (proposalAction === ProposalAction.COLLECT_PROTOCOL) {
      return Boolean(!toAddressValue || !recipientAddressValue || titleValue === '')
    }
  ...
  console.group('>> HAS ENOUGH VOTES')
  console.log('Available Votes:', availableVotes?.quotient.toString())
  console.log('Proposal Threshold:', proposalThreshold?.quotient.toString())
...
`
    ...
    switch (proposalAction) {
      ...
      case ProposalAction.SET_FEE_PROTOCOL: {
        createProposalData.targets = [toAddressValue]
        types = [['uint8', 'uint8']]
        values = [['10', '10']]
        createProposalData.signatures = [`setFeeProtocol(${types[0].join(',')})`]
        break
      }

      case ProposalAction.COLLECT_PROTOCOL: {
        createProposalData.targets = [toAddressValue]
        types = [['address', 'uint128', 'uint128']]
        values = [[recipientAddressValue, '999999000000000000000000', '999999000000000000000000']]
        createProposalData.signatures = [`collectProtocol(${types[0].join(',')})`]
        break
      }
    }
    ...
    console.group('>> CREATE PROPOSAL')
    console.log('Create Proposal Data:', createProposalData)
    console.groupEnd()
        ...
        <AppBody {...{ maxWidth: '800px' }}>
          ...
          <CreateProposalWrapper>
            ...
            <ProposalActionDetail
              ...
              onRecipientAddressInput={handleRecipientAddressInput}
            />
            <ProposalEditor
              title={titleValue}
              body={bodyValue}
