...
const Bag = () => {
  ...
  const { totalEthPrice, totalUsdPrice } = useMemo(() => {
    ...
    const totalUsdPrice = fetchedPriceData
      ? parseFloat(formatEther(totalEthPrice)) * fetchedPriceData
      : undefined;
  ...
  return (
    <>
      {bagExpanded && shouldShowBag ? (
        <Portal>
          <Column
            zIndex={isMobile || isOpen ? "modal" : "3"}
          ...
          >
            {!(
              ...
              <>
                ...
                {userCanScroll && scrollProgress > 0 ? (
                  <ScrollingIndicator top show={true} />
                ) : null}
                <Column
                  ref={scrollRef}
